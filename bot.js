...
hours = 12;
  ...
  if(message.content.startsWith(prefix + "giveaway")) {
    ...
    message.channel.send(`:eight_pointed_black_star:| **ارسل اسم روم الي تريد فيه الجيف اواي**`).then(msg => {
      message.channel.awaitMessages(filter, {
        ...
        if(!room) return message.channel.send(':heavy_multiplication_x:| **انا لقيت الروم :(**');
          ...
          message.channel.awaitMessages(filter, {
            ...
            msg.edit(':eight_pointed_black_star:| **الشئ الي عليه الجيف اواي**').then(msg => {
              message.channel.awaitMessages(filter, {
                ...
                message.channel.send(`:heavy_multiplication_x:| **ما عندي برمشن الكافية لصنع الجيف اواي**`);
                  console.log(e);
                }
              });
            });
 ...
 if (message.content === "%help-Games") {
  const embed = new Discord.RichEmbed()
	  ...
	  .addField('%لو خيروك', `لعبة لو خيروك`)
          ...
          .addField('%عواصم', `عشان يقول لك اسم بلد و انت تقول عاصمته`)
          .addField('%rps', `للعب لعبة حجر ورقة مقص`)
          .addField('%speed', `للعب لعبة اسرع كتابة`)
...
if (!points[message.author.id]) points[message.author.id] = {
  ...
  if(!message.guild) return;
    ...
    spee[(message.author.id)] = new Date().getTime()
...
if (message.content.startsWith('%bcall')){
 if (message.author.id !== '385103221804236800') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
 if(!message.author.id === '385103221804236800') return;
...
@@ -2399,15 +2395,15 @@ ${videos.map(video2 => `[**${++index} **] \`${video2.title}\``).join('\n')}`)
					console.error(err);
		...
		serverQueue.songs = [];
@@ -2417,7 +2413,7 @@ ${videos.map(video2 => `[**${++index} **] \`${video2.title}\``).join('\n')}`)
		if (!msg.member.voiceChannel) return msg.channel.send('أنت لست بروم صوتي .');
...
@@ -2426,12 +2422,12 @@ ${videos.map(video2 => `[**${++index} **] \`${video2.title}\``).join('\n')}`)
...
@@ -2441,29 +2437,29 @@ ${serverQueue.songs.map(song => `**${++index} -** ${song.title}`).join('\n')}
...
@@ -2472,11 +2468,11 @@ async function handleVideo(video, msg, voiceChannel, playlist = false) {
			songs: [],
			volume: 5,
			playing: true
		};//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
...
@@ -2486,40 +2482,40 @@ async function handleVideo(video, msg, voiceChannel, playlist = false) {
...
@@ -2539,8 +2535,8 @@ if (message.content.startsWith(adminprefix + 'setT')) {
 });
 client.on("message", message => {
...
@@ -2550,10 +2546,12 @@ ${prefix}vol ⇏ لتغيير درجة الصوت 100 - 0
